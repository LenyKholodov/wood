...
static void nearCallback (void *data, dGeomID o1, dGeomID o2)
{
  ...
  if (n > 0) {
	...
	dContactSoftERP | dContactSoftCFM | dContactApprox1;
      ...
      contact[i].surface.soft_erp = 1;   //def 0.9
      contact[i].surface.soft_cfm = 0.1; //def:0.3
...
static void simLoop (int pause)
{
  ...
  if (!pause) {
    ...
    dJointSetHinge2Param (joint[0],dParamFMax,10);
...
int main (int argc, char **argv)
{
  ...
  for (i=1; i<=3; i++) {
    ...
    sphere[i-1] = dCreateCylinder(NULL,RADIUS,0.01);//dCreateSphere (0,RADIUS);
  ...
  */
  ...
  for (i=0; i<3; i++) {
    ...
    dJointSetHinge2Param (joint[i],dParamSuspensionCFM,0.008);
    dJointSetHinge2Param (joint[i],dParamFudgeFactor,0.01);
  }
