...
#define CMASS 1000	// chassis mass
...
static void command (int cmd)
{
  ...
  case ' ':
	...
	case '2':
		...
		break;
	case '3':
		RotateTrack=RotateTrack?false:true;
		break;
  }
}
...
static void simLoop (int pause)
{
  ...
  if (!pause) {
	...
	if(RotateTrack)
	{
		dQuaternion q;
		dMatrix3 R,final_R;
		dQFromAxisAndAngle(q,0,1,0,M_PI*0.001);
		dQtoR(q,R);
		dMULTIPLY1_333(final_R,R,dGeomGetRotation(mesh));
		dGeomSetRotation(mesh,final_R);
	}

	for(int i=0;i<STEP_IN_COUNT;i++)
	{		
		dSpaceCollide (space,0,&nearCallback);
		dWorldStep (world,0.05/STEP_IN_COUNT);
  ...
  dsDrawGeom(ground_box);
	...
	if(!StartTime)
		...
		printf("FPS: %f \r",float(Count*CLOCKS_PER_SEC)/float(clock()-StartTime));*/
	clock_t NewTime=clock();
	if(StartTime)
		printf("FPS: %f \r",float(CLOCKS_PER_SEC)/float(NewTime-StartTime));
	StartTime=NewTime;
}
...
int main (int argc, char **argv)
{
  ...
  ground=dCreatePlane (space,0,0,1,0);
	...
	mesh=CreateTorus(space,8,1.8,160,16,true);
	MegaMesh=NULL;//CreateMegaMesh(space,0.2,0.2,16,16);
  ...
  for (i=1; i<5; i++) {
	...
	if(i>4)
	{
		...
		if(i==1) // forward wheel
		{
			...
			sphere[i-1]=dCreateCylinder(NULL,RADIUS,WHEEL_W);//dCreateSphere (0,RADIUS);		
  ...
  */
  ...
  dRFromAxisAndAngle(R,0,1,0,M_PI/6.0);
