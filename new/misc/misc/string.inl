...
template <size_t ssize>
ostream&  operator <<     (ostream& os,const String<ssize>& s)
{
  os<<s.mString;
  return os;  
}
...
template <size_t ssize>
istream&  operator >>     (istream& is,String<ssize>& s)
{
  is.getline (s.mString,s.mSize-1);
  return is;
}

template <size_t ssize>
String<ssize>::String<ssize>  (const char* string)
{   
  if (string)
  {
    int len = strlen (string);
    if (len <= ssize) strcpy (mString,string);
    else
    {
      memcpy (mString,string,len);
      mString [len] = 0;
    }
  }
  else
