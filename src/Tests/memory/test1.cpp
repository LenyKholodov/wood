...
const    clock_t    TEST_TIME           = CLK_TCK*10;
const    clock_t    TEST_POOL_SIZE      = 1024*1024*256;
...
//    int allocSize = Random1024()*Random1024()*(Random1024()>>2);       
    int allocSize = Random1024()*Random1024();  
...
//       pool->defrag ();      
       pool->defrag (allocSize);

       float e = clock ();

       dout<<"Defrag time: "<<(e-s)/float (CLK_TCK)<<endl;
  ...
  cout<<"FPS:\t"<<float(total)/time<<endl; 
