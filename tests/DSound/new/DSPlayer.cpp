...
CDSPlayer::CDSPlayer(HWND hWnd,char *PakName,char *ResTab)
{
	...
	if(hr!=S_OK)
		return;
	hr = DirectSoundCreate(NULL, (LPDIRECTSOUND *)&m_DS, NULL);
	if(hr==S_OK)
	{
	...
	if (hr==S_OK)
	{
		...
		hr=m_PrBuf->QueryInterface(IID_IDirectSound3DListener,(LPVOID *)&m_Listener);
		if(hr!=S_OK)
		{
			m_PrBuf->Release();
		...
		if(hr==S_OK)
		{
			...
			if(hr==E_NOTIMPL )
			{
				MessageBox(NULL,"fgdfgdf","dfgdfgdf",MB_OK);
			}
