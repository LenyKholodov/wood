...
FreeImage_Rescale(FIBITMAP *src, int dst_width, int dst_height, FREE_IMAGE_FILTER filter) {
	if ((src) && (dst_width > 0) && (dst_height > 0)) {
		if (FreeImage_GetBPP(src) == 32) {
			// allocate a new 32-bit DIB
			FIBITMAP *dst = FreeImage_Allocate(dst_width, dst_height, 32, 0xFF, 0xFF00, 0xFF0000);
			...
			RGBQUAD *pSrc = (RGBQUAD *)FreeImage_GetBits(src);
			RGBQUAD *pDst = (RGBQUAD *)FreeImage_GetBits(dst);
