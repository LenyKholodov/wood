...
char *LoadFilterString[] = {"3D Studio (*.3DS)","*.3DS"};

char *SaveFilterString[] = {"RR Model (*.RRM)","*.RRM",
							"Q2 Modeler (*.ASC)","*.ASC"};		
        ...
        case WM_COMMAND:
            switch( LOWORD(wParam) )
            {
				...
				case IDM_OPEN:
					...
					if(strcmpi(strFileExt,"3DS") == 0) 
					{
						D3DTextr_InvalidateAllTextures();
						pC3DS->Release3DSModel(&k3dsModel);
						pC3DS->Load3dsModel(&k3dsModel, strFile);
						D3DTextr_RestoreAllTextures(m_pd3dDevice);
					
						strcpy(title_bar_string, "D3DS View v2.0 - ");
